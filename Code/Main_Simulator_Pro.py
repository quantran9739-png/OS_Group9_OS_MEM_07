...
class App(ctk.CTk):
    ...
    def load_blocks_csv(self):
        # DÙNG askopenfilename ĐỂ MỞ FILE
    ...
    def run_simulation(self):
        ...
        algo, stats = self.algo_menu.get(), {"success": 0, "fail": 0, "used": 0}
        ...
        def process_loop(idx):
            ...
            if algo == "First-Fit":
                for i, b in enumerate(self.memory_blocks):
                    if b['p'] is None and b['size'] >= p['size']: target_idx = i; break
            elif algo == "Best-Fit":
                min_d = float('inf')
                for i, b in enumerate(self.memory_blocks):
                    if b['p'] is None and b['size'] >= p['size'] and (b['size'] - p['size']) < min_d:
                        min_d = b['size'] - p['size']; target_idx = i
            elif algo == "Worst-Fit":
                max_d = -1
                for i, b in enumerate(self.memory_blocks):
                    if b['p'] is None and b['size'] >= p['size'] and (b['size'] - p['size']) > max_d:
                        max_d = b['size'] - p['size']; target_idx = i
            if target_idx != -1:
    ...
    def export_csv(self):
        ...
        # DÙNG asksaveasfilename ĐỂ LƯU FILE MỚI
