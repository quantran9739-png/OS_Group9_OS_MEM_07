...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("NHÓM OS_MEM_07 - Worst-Fit Simulator")
        ...
        # Nhãn thuật toán WORST-FIT (Màu Cam)
        ctk.CTkLabel(self.sidebar, text="THUẬT TOÁN: WORST-FIT", font=("Roboto", 14, "bold"), fg_color="#e67e22", corner_radius=5).pack(pady=10, padx=20, fill="x", ipady=5)
    ...
    def update_speed(self, val): self.sim_speed = int(val)
    ...
    def run_simulation(self):
        ...
        def process_loop(idx):
            ...
            # Khóa cứng logic Worst-Fit
            if algo == "Worst-Fit":
                max_d = -1
                for i, b in enumerate(self.memory_blocks):
                    if b['p'] is None and b['size'] >= p['size'] and (b['size'] - p['size']) > max_d:
                        max_d = b['size'] - p['size']
                        target_idx = i
    ...
    def export_csv(self):
        ...
        path = filedialog.asksaveasfilename(defaultextension=".csv", initialfile="ket_qua_mo_phong_WorstFit.csv", filetypes=[("CSV Files", "*.csv")])
        if path:
            try:
            ...
            except Exception as e: messagebox.showerror("Lỗi", f"Không thể lưu: {e}")

    def reset_simulation_data(self):
        self.results = []
        for b in self.memory_blocks: b['p'] = None; b['size'] = b['original_size']
        for child in self.table_container.winfo_children():
            if child != self.header_frame: child.destroy()
        self.draw_blocks()

    def reset_app(self):
        self.memory_blocks = []; self.process_list = []; self.reset_simulation_data()
